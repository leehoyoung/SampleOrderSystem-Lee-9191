...
3. **컨트롤러의 직접 빌드/재실행 검증도 PR 단위로 1회가 기본이다** — 태스크마다
   ...
   예외 태스크는 그 시점에도 함께 검증한다. **반드시 테스트 프로젝트뿐 아니라 전체
   솔루션(`SampleOrderSystem.slnx`)을 빌드한다** — 테스트 프로젝트만 빌드하면 메인
   애플리케이션 프로젝트(`SampleOrderSystem.vcxproj`)가 인터페이스 변경(예: 생성자
   시그니처 변경)을 따라가지 못해 깨진 채로 넘어갈 수 있다(order-intake 태스크
   2~4에서 실제로 발생 — `Application.cpp`가 갱신되지 않아 메인 앱만 빌드 실패).
...
8. **서브에이전트에게 위임하는 프롬프트에는 관련 파일 경로(가능하면 라인 번호)를
   ...
   탐색 스텝을 줄인다. 파일이 작거나 중간 크기면 경로/라인만 짚어주는 데 그치지 않고
   **컨트롤러가 미리 읽은 관련 코드 스니펫을 프롬프트에 직접 인용**한다 — 에이전트의
   Read/Grep 왕복 자체를 없애 체감 시간을 줄인다 (파일이 크면 전체를 붙여넣지 않고
   경로/라인 안내로 충분히 대체한다).
   (2026-07-15, 사용자 확인)
8-1. **같은 PR 안에서 같은 역할(test-writer 또는 implementer)의 다음 배치를 이어갈
   때는 새 에이전트를 띄우지 않고 `SendMessage`로 직전 에이전트 세션을 재개한다.**
   이미 스킬 문서/관련 파일을 읽은 컨텍스트를 재사용해 반복 탐색을 없앤다. 단,
   **RED(test-writer)와 GREEN(implementer)은 절대 같은 에이전트로 합치지 않는다** —
   이 역할 분리 자체가 "test-writer가 진짜 실패를 직접 확인한 뒤에만 implementer가
   로직을 채운다"는 품질 게이트이기 때문이다(예: order-intake 태스크 1처럼 이미
   GREEN인 경우를 이 분리 덕분에 걸러낼 수 있었다). 세션은 PR이 바뀌면(다른 기능
   영역으로 넘어가면) 새로 시작한다. (2026-07-15, 사용자 확인)
